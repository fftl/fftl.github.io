...
이제 이 상황에서 주어지는 질문은 다음과 같습니다. [[백업 case1]]

1. 각 고객이 레스토랑에서 쓴 총 금액은 얼마입니까?
```sql
select m.customer_id, sum(price) as total_cost
from dannys_diner.members m, dannys_diner.sales s, dannys_diner.menu mn
...
from dannys_diner.members m, dannys_diner.sales s
where m.customer_id = s.customer_id
group by m.customer_id

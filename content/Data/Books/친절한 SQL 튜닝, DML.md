...
modified: 2026-01-18T23:31:35+09:00
---
# DML 튜닝
...
- 파티션 Truncate를 이용한 대량 데이터 삭제
	- ```sql 
		...
		```
	- 위와 같이 삭제 조건이 여러 개일 경우 조건을 만족하는 데이터가 소수라면 그대로 사용하면 좋지만, 조건을 만족하는 데이터가 대다수라면 대량 데이터를 지우는 것이 아니라 남길 데이터만 백업했다가, 재입력하는 방식이 빠르다. 아래와 같은 순서로 처리한다.
	1. 임시 테이블(거래_t)을 생성하고, 남길 데이터만 복제한다.
	2. 삭제 대상 테이블 파티션을 Truncatre 한다.
...
비파티션 테이블에 손익분기점을 넘는 대량 INSERT를 하려면 인덱스를 Unusable 시켰다가 재생성하는 방식이 더 빠를 수 있다.
1. (가능하다면) 테이블을 nologging 모드로 전환한다.
2. 인덱스를 Unusable 상태로 전환한다.
3. (가능하다면 Direct Path Insert 방식으로) 대량 데이터를 입력한다.
4. (가능하다면, no logging 모드로) 인덱스를 재생성한다.
5. (nologging 모드로 작업했다면) logging 모드로 전환한다.

테이블이 파티셔닝돼 있고, 인덱스도 로컬 파티션이라면, 파티션 단위로 인덱스를 재생성할 수 있기 때문에 효율적으로 입력할 수 있다.
